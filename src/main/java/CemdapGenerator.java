...
public class CemdapGenerator {
    ...
    private String[] generateCemdapData(int n) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            double Age;
            {
                int gauss = (int) Math.round(r.nextGaussian() * 26 + 40);
                if (gauss > 100) {
                    Age = 100;
                } else if (gauss < 18) {
                    continue; // We are not counting children now.
                } else {
                    Age = gauss;
                }
            }
            // adult or child is a student
            double stu = (Age > 18 && r.nextDouble() < 0.02) ? 1.0 : 0.0;
            // adult is employed
            ...
            double somecol = (Age > 22 && r.nextDouble() < 0.3) ? 1 : 0;
            // associate or bachelors degree completed, adult
            double assobach = (somecol == 1 && r.nextDouble() < 0.2) ? 1 : 0;
        ...
        write("original-input-data/plymouth/persons.sql", outp);
        write("original-input-data/plymouth/households.sql", outh);
        write("original-input-data/plymouth/zones.sql", outz);
        write("original-input-data/plymouth/zone2zone.sql", outz2z);
        write("original-input-data/plymouth/losoffpk.sql", outLos);
